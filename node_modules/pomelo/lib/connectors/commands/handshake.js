...
var response = function(socket, sys, resp) {
  ...
  // console.log(res)
  var splittedChunks = JSON.stringify(res).match(/.{1,800}/g);
  var chunkData = {};
  chunkData.chunks = splittedChunks.length;
  chunkData.last = false;
  for (var i = 0; i < splittedChunks.length; i++) {
    chunkData.index = i+1;
    chunkData.data = splittedChunks[i];
    chunkData.last = (splittedChunks.length-1)==i;
    socket.handshakeResponse(Package.encode(Package.TYPE_HANDSHAKE, new Buffer(JSON.stringify(chunkData))), chunkData.last);
  }
};
